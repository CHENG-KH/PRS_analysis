...
data=${pathway}Input/Laptop_input/"$phenoarray"_base.txt
...
${pathway}Input/Script/bin/plink --vcf process/indQC_nodup_chr_xym_"$phenoarray".vcf --pheno ${pathway}Input/Laptop_input/"$phenoarray"_base.txt --pheno-name $phenoarray --make-bed --out $phenoarray/"$phenoarray"_p
