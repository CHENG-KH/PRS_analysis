...
urPhen <- "hypertension" # Type phenotype here
phenotype <- read.table("/Users/kenny/Desktop/Laptop_input/hypertension_target.txt", header=T) %>%
  mutate(hypertension = hypertension - 1)
...
pcs <- fread("/Users/kenny/Desktop/Laptop_input/pcaOut.eigenvec", header=F) %>%
    setnames(., colnames(.), c("FID", "IID", paste0("PC",1:20))) # choose all PCs
pheno <- phenotype %>%
  merge(., pcs)
colnames(pheno)[3] <- 'mypheno'

#Choose 2 PC due to the scree plot#####
pheno <- pheno[,c(1:6)] # 3 PC
